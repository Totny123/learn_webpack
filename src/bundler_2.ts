...
// 项目绝对路径
const projectRootDir = path.resolve(__dirname, './bundler_2');
// 声明依赖图的类型
...
const collect = (entryDir: string) => {
  ...
  // 转成es5代码
  const es5Result = babel.transform(code, {
...
const getDistString = () => {
  return `
  const depRelation = ${transformDepRelation(depRelation)}
  // 缓存模块的导出
  const modulesCache = {};
  const execute = (key) => {
    if (!key) return;
...
const bundler = () => {
  ...
  fs.rmdirSync(path.resolve(process.cwd(), 'dist'), { recursive: true });
  fs.mkdirSync(path.resolve(process.cwd(), 'dist'), { recursive: true });
  fs.writeFileSync(path.resolve(process.cwd(), 'dist/index.js'), distString);
};
