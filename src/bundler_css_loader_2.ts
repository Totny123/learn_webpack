...
import cssLoader from './loaders_2/css-loader';
import styleLoader from './loaders_2/style-loader';
...
const collect = (entryDir: string) => {
  ...
  if (/\.css$/.test(entryDir.toLowerCase())) {
    transformedCode = cssLoader(transformedCode);
    transformedCode = styleLoader(transformedCode);
  }
  // 源文件相对根目录的路径
